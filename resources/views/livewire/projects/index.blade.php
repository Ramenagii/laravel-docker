    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-8">
            ...
            <button wire:click="$set('showCreateModal', true)"
                    ...
                    wire:target="$set('showCreateModal', true)"
                ...
                <span wire:loading.remove wire:target="$set('showCreateModal', true)">
                    <svg class="w-4 h-4 mr-2 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                    </svg>
                    New Project
                </span>
                <span wire:loading wire:target="$set('showCreateModal', true)" class="flex items-center">
            ...
            <div class="flex flex-col sm:flex-row gap-4">
                ...
                <div class="flex space-x-2">
                    <button wire:click="$set('statusFilter', '')"
                            wire:loading.attr="disabled"
                            wire:target="$set('statusFilter', '')"
                            class="px-4 py-2 rounded-xl text-sm font-medium transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed {{ $statusFilter === '' ? 'bg-indigo-600 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                        @if($statusFilter === '')<span wire:loading.remove wire:target="$set('statusFilter', '')">All</span><span wire:loading wire:target="$set('statusFilter', '')" class="flex items-center"><svg class="animate-spin w-3 h-3 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/></svg></span>@else All @endif
                    </button>
                    <button wire:click="$set('statusFilter', 'active')"
                            wire:loading.attr="disabled"
                            wire:target="$set('statusFilter', 'active')"
                            class="px-4 py-2 rounded-xl text-sm font-medium transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed {{ $statusFilter === 'active' ? 'bg-emerald-600 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                        @if($statusFilter === 'active')<span wire:loading.remove wire:target="$set('statusFilter', 'active')">Active</span><span wire:loading wire:target="$set('statusFilter', 'active')" class="flex items-center"><svg class="animate-spin w-3 h-3 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/></svg></span>@else Active @endif
                    </button>
                    <button wire:click="$set('statusFilter', 'completed')"
                            wire:loading.attr="disabled"
                            wire:target="$set('statusFilter', 'completed')"
                            class="px-4 py-2 rounded-xl text-sm font-medium transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed {{ $statusFilter === 'completed' ? 'bg-blue-600 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                        @if($statusFilter === 'completed')<span wire:loading.remove wire:target="$set('statusFilter', 'completed')">Completed</span><span wire:loading wire:target="$set('statusFilter', 'completed')" class="flex items-center"><svg class="animate-spin w-3 h-3 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/></svg></span>@else Completed @endif
                    </button>
                    <button wire:click="$set('statusFilter', 'archived')"
                            wire:loading.attr="disabled"
                            wire:target="$set('statusFilter', 'archived')"
                            class="px-4 py-2 rounded-xl text-sm font-medium transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed {{ $statusFilter === 'archived' ? 'bg-slate-600 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                        @if($statusFilter === 'archived')<span wire:loading.remove wire:target="$set('statusFilter', 'archived')">Archived</span><span wire:loading wire:target="$set('statusFilter', 'archived')" class="flex items-center"><svg class="animate-spin w-3 h-3 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/></svg></span>@else Archived @endif
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
            ...
            @empty
                <div class="col-span-full bg-white rounded-2xl shadow-sm border border-slate-200 p-12 text-center">
                    ...
                    <button wire:click="$set('showCreateModal', true)" wire:loading.attr="disabled" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-xl hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                ...
                <div class="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full mx-auto overflow-hidden">
                    ...
                    <form wire:submit="create" class="px-6 py-5">
                        ...
                        <div class="flex justify-end space-x-3">
                            ...
                            <button type="submit" wire:target="create" class="px-5 py-2.5 text-sm font-medium text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
