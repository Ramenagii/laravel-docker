<div class="py-8 px-4 lg:px-8">
    <div class="max-w-7xl mx-auto">
        ...
        <!-- Kanban Board with responsive horizontal scroll -->
        <div class="overflow-x-auto pb-2 -mx-4 lg:mx-0">
            <div class="grid grid-cols-4 gap-4 md:gap-5 min-w-[700px] lg:min-w-0 px-4 lg:px-0">
                <!-- Todo Column -->
                <div class="bg-slate-50/80 rounded-2xl border border-slate-200 p-3 md:p-4">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center space-x-2">
                            <span class="w-3 h-3 rounded-full bg-slate-500 animate-pulse"></span>
                    ...
                    <div class="space-y-3 min-h-[80px] md:min-h-[120px]">
                        ...
                        @endforeach
                        <div class="text-center py-4 md:py-6 text-xs text-slate-400">
                            <svg class="w-6 h-6 md:w-8 md:h-8 mx-auto mb-2 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                            </svg>
                            <span class="hidden md:inline">Drop tasks here</span>
                        </div>
                    </div>
                </div>

                <!-- In Progress Column -->
                <div class="bg-blue-50/80 rounded-2xl border border-blue-200 p-3 md:p-4">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center space-x-2">
                    ...
                    <div class="space-y-3 min-h-[80px] md:min-h-[120px]">
                        @foreach($this->inProgressTasks as $i => $task)
                            <div x-data="{ show: false }" x-init="setTimeout(() => show = true, {{ $i }} * 30)" x-show="show" x-transition:enter="transition-all duration-300" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0"
                ...
                <!-- Review Column -->
                <div class="bg-amber-50/80 rounded-2xl border border-amber-200 p-3 md:p-4">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center space-x-2">
                    ...
                    <div class="space-y-3 min-h-[80px] md:min-h-[120px]">
                        @foreach($this->reviewTasks as $i => $task)
                            <div x-data="{ show: false }" x-init="setTimeout(() => show = true, {{ $i }} * 30)" x-show="show" x-transition:enter="transition-all duration-300" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0"
                ...
                <!-- Done Column -->
                <div class="bg-emerald-50/80 rounded-2xl border border-emerald-200 p-3 md:p-4">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center space-x-2">
                    ...
                    <div class="space-y-3 min-h-[80px] md:min-h-[120px]">
                        @foreach($this->doneTasks as $i => $task)
                            <div x-data="{ show: false }" x-init="setTimeout(() => show = true, {{ $i }} * 30)" x-show="show" x-transition:enter="transition-all duration-300" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0"
                                 class="bg-white rounded-xl shadow-sm border border-slate-200 p-3 md:p-3.5 hover:shadow-md hover:border-emerald-200 hover:-translate-y-0.5 transition-all duration-150 group">
                                <div class="flex items-start justify-between">
